...
- **连接**：`lib/providers/aria2_daemon_provider.dart` 按 `ConnectionMode` 选择 `LibraryDaemon`（本机模式，唯一引擎；ADR-007 / ADR-010）或 `RemoteDaemon`（远程 RPC）。
...
- **规划**：`PLAN.md` WBS；用户可见变更同步 `CHANGELOG.md` [Unreleased]；架构决策见 `docs/ARCHITECTURE.md` ADR 表（当前最新 ADR-010「移除 aria2c 子进程引擎」）。
...
- 本机模式只剩 `LibraryDaemon`：**不要**再新增任何依赖 `aria2c` 子进程的逻辑（process spawn、binary resolver、stage 脚本等）。需要外部 aria2c 的场景请走「远程 RPC」连接模式。

## 扩展

- Chrome：`extensions/chrome/` — 选项页可从剪贴板导入 aria2down 复制的 RPC JSON（仅在「远程 RPC」连接模式下有意义）。
